...
function getNextOrder() {
    const index = (__VU - 1) * 10000 + __ITER;
...
export default function () {
    ...
    if (Math.random() < 0.04) {
        ...
        if (order != null) {
            ...
            if (!isOrderSuccess) {
                console.error(`❌ Order Create Failed - Status: ${resOrder.status}, Body: ${resOrder.body}`);
                errors.add(1);
                return;
            }
            ...
            // 주문 생성 성공 시 → 결제 처리 수행
            try {
                const orderResponse = JSON.parse(resOrder.body);
                const paymentPayload = {
                    orderId: orderResponse.orderId,
                    userId: order.userId
                };

                const resPayment = http.post(`${BASE_URL}${PAYMENT_API}`, JSON.stringify(paymentPayload), {
                    headers: { 'Content-Type': 'application/json' }
                });

                const isPaymentSuccess = check(resPayment, { 'payment 200': (r) => r.status === 200 });
                if (!isPaymentSuccess) {
                    console.error(`❌ Payment Failed - Status: ${resPayment.status}, Body: ${resPayment.body}, OrderId: ${paymentPayload.orderId}, UserId: ${paymentPayload.userId}}`);
                    errors.add(1);
                }
            } catch (e) {
